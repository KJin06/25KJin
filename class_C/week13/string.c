...
int login(int num) {
	...
	scanf("%s", tmpPWD);
	for (int i = 0; i < num; i++) {
		if (strcmp(accounts[i].id, tmpID) == 0) {
			if (strcmp(accounts[i].pwd, tmpPWD) == 0) {
				printf("로그인 성공\n");
				loginFlag = i;
				return 1;
			}
...
int join(int num) {
	printf("ID : ");
	scanf("%s", accounts[num].id);
	printf("Pwd : ");
	scanf("%s", accounts[num].pwd);
...
int printBankMenu() {
	int tmp;
	printf("\n\nlogin user %s", accounts[loginFlag].id);
	printf("\n--Catholic Bank--\n");
	printf("1. 입금\n");
	printf("2. 출금\n");
	printf("3. 조회\n");
...
int bankLogin() {
	int tmp, TorF = 0, transferFlag, recordNum = 0;
	record loginRecord[10];
	char tmpList[10], transferID[20];
	...
	do
	{
		...
		case transfer: {
			...
			for (int i = 0; i < numOfAccounts; i++) {
				if (strcmp(accounts[i].id, transferID) == 0) {
					TorF = 1;
					transferFlag = i;
				}
			}
			...
			loginRecord[recordNum].amount = tmp;
			loginRecord[recordNum].balance = accounts[loginFlag].money;
			sprintf(tmpList, "%s", accounts[transferFlag].id);
